...
export const getUsersAndTripsStats = async () => {
  ...
  const filterUsersByRole = (role) => {
    return users.documents.filter((u) => u.status === role);
  };
...
export const getUserGrowthPerDay = async () => {
  ...
  const userGrowth = users.documents.reduce((acc, user) => {
...
export const getTripsCreatedPerDay = async () => {
  ...
  const tripsGrowth = trips.documents.reduce((acc, trip) => {
...
export const getTripsByTravelStyle = async () => {
  ...
  const travelStyleCounts = trips.documents.reduce((acc, trip) => {
